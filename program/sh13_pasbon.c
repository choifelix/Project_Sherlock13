...
char gbuffer[256]; //les messages recus depuis le server
char gServerIpAddress[256];  //l'adresse du serveur
int gServerPort;   //le port du serveur
char gClientIpAddress[256];  //l'adresse ip du joueur
int gClientPort;   //port du joueur
char gName[256]; //nom du joueur
char gNames[4][256]; //liste des noms des joueuers 
int gId; //Id fu joueur
int joueurSel;
int objetSel;
int guiltSel;
int guiltGuess[13];  //la liste des suspect
int tableCartes[4][8];  // tableau des objets
int b[3];  //cartes attribuées au joueur
int goEnabled;  //si tu peux jouer ou pas
int connectEnabled; //si t'es connecté au serveur de jeu
...
int main(int argc, char ** argv){
	...
	for (i=0;i<13;i++)
		guiltGuess[i]=0; //aide graphique pour le joueur, 1 raye et 0 ne raye pas
   ...
   ret = pthread_create ( & thread_serveur_tcp_id, NULL, fn_serveur_tcp, NULL);
	...
	puts("loop");
		...
		//printf("un event\n");
			...
			//printf("mx=%d my=%d\n",mx,my);
				if ((mx<200) && (my<50) && (connectEnabled==1)){ //boutton connection 
					sprintf(sendBuffer,"C %s %d %s",gClientIpAddress,gClientPort,gName);
					sendMessageToServer(argv[1], gServerPort, sendBuffer);
...
/*				else if ((mx>=0) && (mx<200) && (my>=90) && (my<330)){ //selection d'un joueur
				...
				else if ((mx>=200) && (mx<680) && (my>=0) && (my<90)){ // selection d'un onjet
					objetSel=(mx-200)/60;
					guiltSel=-1;
				}
				else if ((mx>=100) && (mx<250) && (my>=350) && (my<740)){ // selection d'un suspect
					joueurSel=-1;
					objetSel=-1;
					guiltSel=(my-350)/30;
				}
				else if ((mx>=250) && (mx<300) && (my>=350) && (my<740)){ // raye ou déraye le personnage selectioné
					int ind=(my-350)/30;
